...
cout<<"MATCH: "<<name<<" status: "<<status<<", mode: "<<behavior<<", prgstatus: "<<prgstatus<<" cycle: "<<cycle<<"("<<cycleactive<<")"<<endl;
...
cout<<"EC: "<<n<<":"<<mincond<<", PR: "<<prcondition<<":"<<hsrv::double2a(prcondition_time)<<", TC: "<<tcondition<<":"<<hsrv::double2a(tcondition_time)<<", FC: "<<fcondition<<":"<<hsrv::double2a(fcondition_time)<<endl;
    if(!evcondition && !cycleactive) return false;
    if(mincond>0 && current_time <latentUntil) return false;
cout<<"test1-passed: "<<mincond<<" "<<prcondition<<" "<<tcondition<<" "<<fcondition<<" ct:"<<hsrv::double2a(current_time)<<endl;
cout<<"test: "<<(mincond==0 && !cycleactive)<<(prcondition_time < current_time)<<(tcondition_time < current_time)<<(fcondition_time < current_time)<<endl;
