...
public:
     ...
     */
    std::string name;
    bool match(std::vector<std::string>& active,bool pgrstatus, EventCounterSet*, BooleanConditionSet*, setof<MMessage>&);
    bool init(); 
...
private:
    ...
    bool isRuleActive(std::vector<std::string>&);
    bool isNotMember(std::string s, std::vector<std::string>& active);
};
