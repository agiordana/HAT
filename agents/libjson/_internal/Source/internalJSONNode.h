...
public:
    ...
    mutable unsigned char _type BITS_JSON(3);

    json_string _name;
    mutable bool _name_encoded BITS_JSON(1);  //must be above name due to initialization list order

    mutable json_string _string;   //these are both mutable because the string can change when it's fetched
    mutable bool _string_encoded BITS_JSON(1);

    //the value of the json
    union value_union_t {
	   bool _bool BITS_JSON(1);
	   json_number _number;
	   #ifdef JSON_LESS_MEMORY
    ...
    #if !defined(JSON_PREPARSE) && defined(JSON_READ_PRIORITY)
	   mutable bool fetched BITS_JSON(1);
    #endif
