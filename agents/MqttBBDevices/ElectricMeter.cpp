...
bool ElectricMeter::publish() {
    ...
    if(hsrv::mqtt == "ON") {
        MMessage mqtt_mess = mkMqttmessage("event", name, "value="+hsrv::double2a(Ipower), "");
	mqtt_mess.add("energy", hsrv::double2a(energy));
        mqtt_mess.add("Ipower", hsrv::double2a(Ipower));
        mqtt_mess.add("Apower", hsrv::double2a(Apower));
        mqtt_mess.add("unit", unit);
        mqtt_mess.add("timeofday", hsrv::getasciitimecompact());
