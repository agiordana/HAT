...
bool RGBPwm::execcmd(MMessage& msg) {
	...
	if(msg.getString("tag")!="") {
	   if(outputStatus=="ON") hsrv::rpcNotify(msg.getString("tag"), "value", ("value="+currentIntensity+",color="+currentColor));
        ...
        if(hsrv::mqtt == "ON") {
	    MMessage mqtt_mess;
            if(outputStatus=="ON") mqtt_mess = mkMqttmessage("event", name, ("value="+currentIntensity+",color="+currentColor), msg.getString("mqtttag"));
	    else mqtt_mess = mkMqttmessage("event", name, ("value=0,color="+currentColor), msg.getString("mqtttag"));
