...
bool TriStateOutput::execcmd(MMessage& msg) {
	...
	hsrv::rpcNotify(msg.getString("tag"), "value", ("value=none"));
        if(hsrv::mqtt == "ON") {
            MMessage mqtt_mess = mkMqttmessage("event", name, "value=none", msg.getString("mqtttag"));
	...
	if(msg.getString("tag") != "") hsrv::rpcNotify(msg.getString("tag"), "value", ("value="+newstate));
        if(hsrv::mqtt == "ON" ) {
            MMessage mqtt_mess = mkMqttmessage("event", name, "value="+newstate, msg.getString("mqtttag"));
            SubjectSet::notify(mqtt_mess);
        }
