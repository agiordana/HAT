...
bool Rule::match(EventCounterSet* ec, BooleanConditionSet* cnd, setof<MMessage>& answer) {
    string source = "";
    string video = "";
    string comment = "";
    string latency = "";
...
// add enabled actions 
    for(j=0; j<action.size(); j++) {
	   ...
	   answer[i].add("source", source);
	   answer[i].add("comment", comment);
           if(get("delay") != "") answer[i].add("delay",computeDelay(j));
	   i++;
    }
    return true;
}
...
string Rule::computeDelay(unsigned n) {
   string adelay = get("delay");
   if(adelay == "") return "0";
   double bdelay = hsrv::a2double(adelay);
   double delay = bdelay * n;
   return hsrv::double2a(delay);
}
