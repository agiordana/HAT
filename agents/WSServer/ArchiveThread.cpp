...
ArchiveThread::ArchiveThread(string name) {
    ...
    trace->load();
    disaggregate = new MParams("disaggregate");
    disaggregate->load();
    userlog = new UserLog(params);
}
...
//    obj->purge();
    while(1) {
        ...
        if(msg.mtype == "trigger" && msg.msubtype == "archiveupd") obj->update();
        else if(msg.mtype == "logcmd" && msg.msubtype == "archive") {
            feature = msg.getString("feature");
            agent = msg.getString("agent");
		...
		if(isToDisaggregate(msg.getString("class"))) {
		    lfile += "/";
		    lfile += agent;
		    if (!FileManager::isDir(lfile)) {
			FileManager::makeDir(lfile, false);
                ...
                lfile += "/data";
                if(isInCurrentDay(msg.getString("timeofday"))) writelog(lfile, msg);
		else hsrv::logger->info("Out of time message arrived: "+msg.xmlEncode(0));
...
bool ArchiveThread::isToDisaggregate(string cl) {
    if(disaggregate == NULL) return false;
    if(disaggregate->get("all") == "yes") return true;
    if (disaggregate->get(cl) == "yes") return true;
    return false;
}

bool ArchiveThread::isInCurrentDay(string daytime) {
    if(daytime == "") return true;
    string dday = "20"+daytime.substr(0,2) +"_" +daytime.substr(2,2) + "_" + daytime.substr(4,2);
    if(day == dday) return true;
    else return false;
}
