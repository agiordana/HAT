...
MMessage AutomatismManager::execPost(vector<string>& params, string& method, string body) {
   ...
   string msg;
   string itembody;
   BOOST_FOREACH(boost::property_tree::ptree::value_type &v, pt.get_child("automatisms")) {
      string status = v.second.get<string>("enable_status");
      string name = v.second.get<string>("name");
      string from = cvtimej2x(v.second.get<string>("start"));
      string to = cvtimej2x(v.second.get<string>("end"));
